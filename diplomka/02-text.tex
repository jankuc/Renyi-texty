...
\begin{definition}
%	Řekneme, že posloupnost náhodných veličin $\lbrace X_n \rbrace$ s 
...
%Nech\v{t} jsou $X_1,\dots,X_n$ stejně a nezávisle rozdělené reálné ná\-ho\-dné 
...
%
%\begin{equation}
%P\left(\sup_{x\in\mathbb{R}}|F(x)-F_n(x)|>\varepsilon\right) \leq 
$8(n+1)\exp{\left\{ \frac{-n{\varepsilon}^2}{32}\right\}}.$
%\end{equation}
%Z Borel-Cantelliho lemma pak plyne
%\begin{equation}
%\lim_{n \rightarrow +\infty}\sup_{x \in \mathbb{R}}|F(x)-F_n(x)|=0  \quad 
$\text{s. j.}$
%\end{equation}
%\end{theorem}
