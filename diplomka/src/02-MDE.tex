...
As an example of non-parametric estimator of cumulative distribution function (CDF) we can use empirical distribution function (ECDF).
\begin{definition}
	Let $(X_1, \ldots,X_n)$ be i.i.d. random sample. Then empirical distribution function $F_n(x)$ is defined as 
	\begin{equation}
	F_n(x) = \frac{1}{n} \sum_{i=1}^n I_{X_i \leq x} \quad \forall x \in \mathbb{R},
		\label{eq:ecdf}
	\end{equation}
	where $I_{X_i \leq x}$ is the indicator function defined as 
	\begin{equation}
	I_{X_i \leq x} = 
		\begin{cases}
...
Glivenko-Cantelli theorem states limit property of ECDF:
...
\begin{definition}
	Let us have the assumptions from previous definition. Moreover, let us have vector of sample weights $(w_1, \ldots,w_n) \in \mathbb{R}$. Then weighted empirical distribution function (WECDF) $F^w_n(x)$ is defined as 
	\begin{equation}
