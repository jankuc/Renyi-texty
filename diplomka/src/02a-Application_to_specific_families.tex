...
\begin{equation}
	\mathrm{IF}(x;T_{\mathfrak{R}_\alpha},\hat{\mu}) = (1+\alpha )^{\frac{3}{2}} (x-\hat{\mu} )  \exp \left[{-\frac{\alpha}{2} (x-\hat{\mu} )^2}\right]. % IF(x,mu)
	\label{IF-laplace-mu}
\end{equation}
If we switch the role of the parameters, meaning that we estimate the scale $\hat{\theta} = \lambda$, while fixing $ \mu$, we achieve to
\begin{equation}
	\mathrm{IF}(x;T_{\mathfrak{R}_\alpha},\hat{\lambda}) = (1 + \alpha)^2 \left(-\hat{\lambda} + (1 + \alpha)|x-\mu|\right)  \exp\left[{-\frac{\alpha|x-\mu|}{\hat{\lambda}}}\right]	. % IF(x,sigma)
...
\begin{equation}
	\mathrm{IF}(x;T_{\mathfrak{R}_\alpha},\hat{\lambda}) =	(1+\alpha )^2 \left( - \hat{\lambda} +(1+ \alpha)(x-\mu)\right) \exp \left[{-\frac{\alpha (x-\mu)}{\hat{\lambda} }}\right]. % IF(x,lambda),
