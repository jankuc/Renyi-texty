...
\end{align}
Protože podmínka \eqref{beta-podminka} platí pro každé $\beta>0$, takže pak pro $\alpha>0$ můžeme  min $\mathfrak{R}_\alpha$-odhad \eqref{Renyi-estimator_formula} parametrů Laplaceova rozdělení počítat jako 
\begin{equation}
...
Nyní pro {\mRao} v Laplaceově modelu spočítáme influenční funkce podle \eqref{IF}. Pro odhad polohy $\theta = \mu$ při známém rozptylu $\lambda$, nám vychází ve tvaru

\begin{equation}
	\mathrm{IF}(x;T_{\mathfrak{R}_\alpha},\mu) = (1+\alpha )^{\frac{3}{2}} (x-\mu )  e^{-\frac{\alpha}{2} (x-\mu )^2}. % IF(x,mu)
	\label{IF-laplace-mu}
\end{equation}
Pokud prohodíme úlohu parametrů, tedy pokud bude hledaným parametrem rozptyl $\theta = \lambda$ a polohu $ \mu$ budeme považovat za známou, dostaneme 
\begin{equation}
...
Influenční funkce je podle $\eqref{IF}$ v případě odhadovaného parametru polohy $\theta = \mu$ a známého rozptylu $ \lambda$ stejná jako pro Laplaceovo rozdělení $\eqref{IF-laplace-mu}$, tedy 

\begin{equation}
	\mathrm{IF}(x;T_{\mathfrak{R}_\alpha},\mu) = (1+\alpha )^{\frac{3}{2}} (x-\mu )  e^{-\frac{\alpha}{2} (x-\mu )^2}. % IF(x,mu)
	\label{IF-exponential-mu}
\end{equation}
Pokud je hledaným parametrem naopak rozptyl $\theta = \lambda$ při známé poloze $ \mu $, vychází funkce ve tvaru
\begin{equation}
...
Dalším rozdělením, na které jsme použili Rényiho odhady, je Cauchyovo s parametrem $\theta = (\mu,\sigma)$ a hustotou pravděpodobnosti
\begin{equation}
